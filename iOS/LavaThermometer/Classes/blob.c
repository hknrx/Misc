 ...
 * <li>animationTimer Animation timer;</li>
...
void BlobInitialize(GLfloat left, GLfloat right, GLfloat bottom, GLfloat top, unsigned char const shutterLevel)
{
	...
	blobBackground.animationTimer = 0;
...
void BlobLoadTexture(CGImageRef const textureImage, GLuint const textureId, GLuint textureSize, char const*const text)
{
	...
	CGContextRef textureContext = CGBitmapContextCreate(texturePixels, textureSize, textureSize, 8, textureSize * 4, CGImageGetColorSpace(textureImage), kCGImageAlphaNoneSkipLast);
	CGContextDrawImage(textureContext, CGRectMake(0.0f, 0.0f, (CGFloat)textureSize, (CGFloat)textureSize), textureImage);
	if(text)
	{
		size_t textLength = strlen(text);
		CGContextSelectFont(textureContext, "Arial", 18.0f, kCGEncodingMacRoman);
		CGContextSetTextDrawingMode(textureContext, kCGTextInvisible);
		CGContextShowTextAtPoint(textureContext, 0.0f, 0.0f, text, textLength);
		CGPoint textEnd = CGContextGetTextPosition(textureContext);

		CGContextSetLineWidth(textureContext, 1.0f);
		CGContextSetRGBStrokeColor(textureContext, 0.4f, 0.2f, 0.6f, 1.0f);
		CGContextSetRGBFillColor(textureContext, 0.2f, 0.2f, 0.8f, 1.0f);
		CGContextSetTextDrawingMode(textureContext, kCGTextFillStroke);
		CGContextShowTextAtPoint(textureContext, (textureSize - textEnd.x) / 2, (textureSize - 18.0f) / 2, text, textLength);
	}
...
BlobReference BlobObjectCreate(const BlobCharacteristics* characteristics, const BlobShapeReference shapeReference, const BlobTransformation* transformation)
{
	...
	// Set the blob's texture, color, collision mask and timer
	...
	blob->timer = characteristics->timer;
...
static inline void BlobUpdateForce(Blob* blob, unsigned char iteration)
{
	...
	else
	{
		...
		force.y = blob->mass * (++blob->timer & (3 << 12) ? blobGravity.y : -blobGravity.y);
...
static inline void BlobDisplayBackground()
{
	...
	blobBackground.matrix[1] = SIN(++blobBackground.animationTimer);
	blobBackground.matrix[13] = -blobBackground.matrix[1] << (FIXED_POINT_SHIFT - 1);
...
void BlobTextureIdSet(const BlobReference blobReference, unsigned char const textureId)
{
	if(blobReference)
	{
		((Blob*)blobReference)->textureId = textureId;
	}	
}
