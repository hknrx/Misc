...
public class Poker
{
	...
	// Private members
	const int MASK_SUIT = 15 << 15;
	const int MASK_RANK = (1 << 15) - 1;
	static readonly char[] SUITS = new char [] {'H', 'S', 'D', 'C'};
	static readonly char[] RANKS = new char [] {'2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K', 'A'};
	...
	public static List <int> CreateDeck (int wildCountRed, int wildCountBlack)
	{
		...
		while (wildCountRed-- > 0) {
			deck.Add ((1 << 21) | MASK_SUIT);
		}
		while (wildCountBlack-- > 0) {
			deck.Add ((1 << 24) | (1 << 21) | MASK_SUIT);
		}
		return deck;
	}
	...
	public static void Test (int wildCount, int testCount, int displayCount)
	{
		...
		Debug.Log ("Test duration: " + duration + "s\n" +
			"Poker hand check duration: " + durationCheck + "s (= " + (testCount / durationCheck) + " tests/s)");
		int combinationIndex = countCombinations.Length;
		while (combinationIndex-- > 0) {
			Debug.Log ((Combination)combinationIndex + ": " + countCombinations [combinationIndex] + " (" + (countCombinations [combinationIndex] * 100.0f / testCount) + "%)");
		}
	}
