...
// Shader from: https://www.shadertoy.com/view/lsXXz8
...
float getDistance (in vec3 p, out vec4 q) {
	...
	// Ground
	vec3 q_ = p;
	q_.z += 0.8 * (sin (iGlobalTime + p.x * 0.5) + sin (p.y * 0.5));
	...
	q_ = p;
	q_.x -= 5.0;
	q_ = twistY (q_, cos (iGlobalTime), iGlobalTime);
	...
	// Sphere
	q_ = p + vec3 (3.0, -1.0, sin (iGlobalTime * 2.0) - 1.0);
	dist_ = sphere (q_, 1.0);
	if (dist_ < dist) {
		q = vec4 (q_, 3.0);
		dist = dist_;
...
void main () {
	...
	// Old monitor effect
	vec2 frag = 2.0 * gl_FragCoord.xy / iResolution.xy - 1.0;
	...
	gl_FragColor = vec4 (color * colorMonitor, 1.0);
}
